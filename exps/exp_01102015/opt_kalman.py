...
def kalman_opt(kalman_params):
    ...
    # Kalman's filtering algorithm
    kalman_x_p = np.zeros(ap_timed_pred[1].shape)
    kalman_x_p[0, :] = ap_timed_pred[1][0, :]
    kalman_p_m = np.zeros(ap_timed_pred[1].shape)
    kalman_p_p = np.zeros(ap_timed_pred[1].shape)
    kalman_p_p[0, :] = np.ones((1, 4)) * kalman_params[1]

    for i in range(1, ap_timed_pred[1].shape[0]):
        ...
        kalman_x_p[i, :] = kalman_x_p[i-1, :] + kalman_k_i * (ap_timed_pred[1][i, :] - kalman_x_p[i-1, :])

    RSME = np.ones((4))
    for i in range(kalman_p_m.shape[1]):
        RSME_tmp = np.sqrt(np.sum((track.doa_true[1][:, i] - kalman_x_p[:, i]) ** 2) /
                           kalman_x_p.shape[0])
        RSME[i] = RSME_tmp
    return np.mean(RSME)
