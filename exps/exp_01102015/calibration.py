...
for file_name in fnames:
    ...
    """
    Create training data, one for each AP
    """
    arranged_data[cal_index] = fn.arrange_data(dataset[cal_index][:, :-1])
    dataseta_for_sd = pd.DataFrame(dataset[cal_index][:, :-1], columns=['V0', 'V1', 'V2', 'V3', 'H0', 'H1',
                                                                        'H2', 'H3'])

    # filter noise below -10 dB
    arranged_data[cal_index] = fn.noise_filter(np.array(arranged_data[cal_index]))

    # read database from file
    arranged_data[cal_index] = pd.DataFrame(arranged_data[cal_index], columns=['V0', 'V1', 'V2', 'V3', 'H0', 'H1',
                                                                               'H2', 'H3', 'V_m_H'])
    ...
    arranged_data[cal_index] = arranged_data[cal_index].loc[valid_angle]
